...
class Encoder_GRU(nn.Module):
	def __init__(self, input_dim, hid_dim, n_layers, dropout):
		...
		'''
			...
			6、dropout：除最后一个层的每个GRU层后增加一个dropout层，默认为0
			7、bidirectional: 是否为双向GRU.默认False

			8、“dropout option adds dropout after all but last” ，
				当RNN只有一层时，dropout的设置是无效的。因为dropout是在RNN【除最后一层】加上dropout。
		'''
...
class Decoder_GRU(nn.Module):
	'''
		需要输入的是：
			1、encoder部分的最后一层循环输出的[h_n] 作为 decoder的h_0
	...
	def __init__(self, output_dim, hid_dim, n_layers, dropout):
		...
		self.rnn = nn.GRU(output_dim, hid_dim, n_layers, batch_first=False, dropout=dropout)
...
class seq2seq_GRU(nn.Module):
	...
	def forward(self, src, trg, train=True, teacher_forcing_ratio=0.8):
		'''
			真实输入数据格式：
				src = （batch, H_in, seq）
				trg =  (batch, H_out, seq)
		...
		E_output, hidden = self.encoder(src)
...
if __name__ == '__main__':
	...
	# 基本参数
	N_LAYERS = 2        # 模型结构：（encoder-decoder）两层+单向
	ENC_DROPOUT = 0.5
	DEC_DROPOUT = 0.5
	device = 'cpu'
